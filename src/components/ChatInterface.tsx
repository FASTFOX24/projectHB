  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (inputValue.trim() === '안녕 자기소개 해줘') {
      const botMessage: Message = {
        id: Date.now() + 1,
        text: `안녕하세요 저는 호반에서 만든 건설업계 전용 인공지능 어시스턴트, HOBANAI예요. 궁금한 점이 있으면, 무엇이든 빠르고 정확하게 찾아서 알려드릴게요. 😊\n\n주요 특징을 간단히 소개하자면\n\n1. 실시간 사내 정보 제공 : 그룹웨어 공지사항, 사회공헌, 인사발령, 직원소식, 복리후생 등 회사 내의 공지사항을 실시간으로 업데이트하여 알려드릴 수 있어요.\n\n2. 다양한 질문 처리 : 건설 지식과 관련된 간단한 상식부터 복잡한 전문지식까지, 어떤 질문이든 답변해 드릴 수 있어요.\n\n3. 문서/이미지 요약 : 긴 글이나 문서, 이미지도 요약해드려서 빠르게 핵심만 파악할 수 있어요.\n\n앞으로 궁금한 점, 필요한 정보, 고민되는 일 등 언제든 편하게 물어봐주세요!`,
        isUser: false,
      };
      setTimeout(() => {
    ...
    if (inputValue.trim() === '오늘 주문해야 할 새로운 자재나 인력 발주사항 있어?') {
      ...
      const msg2: Message = {
        ...
        text: (
          <span>
            다음주 목요일에(6월 26일) 6동부터 15동 16층부터 최상층(22F~25F)까지 조적 작업이 있습니다. 현재 남아있는 조적용 레미탈 양은 50포(40kg 단위)입니다. '플랜P 현장'의 '00건설' 작업팀 일일 평균 레미탈 시공량은 3포대고, 평균 배송기간은 일주일입니다. 따라서 금일 혹은 명일에 발주 넣는 것을 추천드립니다. 아래 링크를 누르시면 자동으로 발주 주문을 넣어드리겠습니다.<br /><br />
